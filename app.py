...
st.set_page_config(page_title="NDIS Recommender AI", page_icon="♿", layout="wide")
...
def clean_disability_label(value):
    ...
    mapping = {
        ...
        "stroke": "Stroke",
        "visual impairment": "Visual Impairment",
        "dsbltygrp_missing": "Other / Unknown",
        "unknown": "Other / Unknown",
        "missing": "Other / Unknown"
    }

    return mapping.get(lower, text.title())
...
def load_models():
    ...
    return joblib.load(RISK_MODEL_PATH), joblib.load(SUPPORT_MODEL_PATH)


def show_banner():
    for file in [
        ASSETS_DIR / "ndis_banner.jpeg",
        ASSETS_DIR / "ndis_banner.jpg",
        ASSETS_DIR / "ndis_banner.png",
        ASSETS_DIR / "banner.png"
    ]:
        if file.exists():
    ...
    st.markdown(
        """
        <div style="padding:36px;border-radius:18px;background:linear-gradient(135deg,#16002b,#4b006e,#702082);color:white;">
            <h1>NDIS Recommender AI</h1>
            <p>Explainable recommendation and decision-support system for disability service access.</p>
        </div>
        """,
        unsafe_allow_html=True
    )
...
def risk_components(profile):
    count = float(profile["participant_count"])
    providers = max(float(profile["active_provider_count"]), 1)
    utilisation = float(profile["utilisation_rate"])
    budget = float(profile["average_support_budget"])

    ratio = count / providers
    gap = 100 - utilisation

    provider_scarcity = 100 if providers <= 5 else 70 if providers <= 15 else 45 if providers <= 30 else 15

    return pd.DataFrame({
        "Risk Factor": ["Demand Pressure", "Utilisation Gap", "Provider Scarcity", "Budget Complexity"],
        "Score": [
            min(ratio / 100 * 100, 100),
            min(gap, 100),
            provider_scarcity,
            min(budget / 70000 * 100, 100)
        ]
    })
...
def get_top_support_predictions(model, input_df, top_n=3):
    try:
        ...
        out = pd.DataFrame({
            "AI Recommended Support": classes[top],
            "Recommendation Strength": probs[top] * 100
        })
        out["Recommendation Strength"] = out["Recommendation Strength"].round(1).astype(str) + "%"
        return out
    except Exception:
        return pd.DataFrame({
            "AI Recommended Support": [model.predict(input_df)[0]],
            "Recommendation Strength": ["Not available"]
        })
...
def find_similar_groups(df, profile, top_n=5):
    ...
    temp["similarity_score"] = 0.0
    temp["similarity_score"] += (temp["state"].astype(str) == profile["state"]).astype(int) * 2
    temp["similarity_score"] += (temp["age_group"].astype(str) == profile["age_group"]).astype(int) * 2
    temp["similarity_score"] += (temp["disability_type"].astype(str) == profile["disability_type"]).astype(int) * 3

    for col in ["participant_count", "average_support_budget", "utilisation_rate"]:
        temp[col] = pd.to_numeric(temp[col], errors="coerce").fillna(0)

    temp["similarity_score"] -= abs(temp["participant_count"] - profile["participant_count"]).rank(pct=True)
    temp["similarity_score"] -= abs(temp["average_support_budget"] - profile["average_support_budget"]).rank(pct=True)
    temp["similarity_score"] -= abs(temp["utilisation_rate"] - profile["utilisation_rate"]).rank(pct=True)
    ...
    return temp.sort_values("similarity_score", ascending=False)[cols].head(top_n)
...
st.info(
    "This app is a group-level NDIS decision-support prototype. "
    "It does not make individual funding decisions."
)
...
with tab1:
    ...
    with col_img:
        for image in [ASSETS_DIR / "accessibility.jpg", ASSETS_DIR / "accessibility.png"]:
            if image.exists():
                st.image(str(image), caption="Disability service access and support planning", use_container_width=True)
                break

    with col_text:
        st.write("""
        This project addresses a real-world disability service access problem. 
        NDIS participants may have approved funding but still experience difficulty accessing services because of:
        - provider availability
        - low plan utilisation
        - regional service gaps
        - disability-specific support needs
        - complex support categories
        """)

        st.write("""
        The system integrates:
        1. Machine learning for access-risk comparison  
        2. Rule-based reasoning for transparent classification  
        3. Recommendation logic for support suggestions  
        4. Visual analytics for service planning  
        """)

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Rows in Dataset", f"{len(df):,}")
    c2.metric("States/Territories", df["state"].nunique())
    c3.metric("Age Groups", df["age_group"].nunique())
    c4.metric("Disability Groups", df["disability_type"].nunique())
    ...
    st.dataframe(pd.DataFrame({
        ...
        "Description": [
            "NDIS participant, provider, utilisation and payment data are collected.",
            "Raw columns are standardised and merged.",
            "Features such as utilisation gap and participant-to-provider ratio are created.",
            "The system predicts low, medium or high service access risk.",
            "The system recommends support planning actions.",
            "Rule-based reasoning explains why the recommendation was made."
...
with tab2:
    ...
    display_df = df.rename(columns={
        ...
        "participant_count": "Participant Count",
        "average_support_budget": "Average Support Budget",
        "utilisation_rate": "Utilisation Rate",
        "active_provider_count": "Active Provider Count",
        "participant_to_provider_ratio": "Participant-to-Provider Ratio",
        "utilisation_gap": "Utilisation Gap",
    ...
    with c1:
        st.subheader("Access Risk Counts")
        st.dataframe(df["access_risk"].value_counts().reset_index().rename(
            columns={"access_risk": "Count", "index": "Access Risk"}
        ), use_container_width=True)

    with c2:
        st.subheader("Top Disability Groups")
        st.dataframe(df["disability_type"].value_counts().head(15).reset_index().rename(
            columns={"disability_type": "Count", "index": "Disability Group"}
        ), use_container_width=True)
...
with tab3:
    ...
    with c1:
        ...
        st.plotly_chart(px.bar(
            ...
            y="Disability Group",
            orientation="h",
            title="Top Disability Groups"
        ), use_container_width=True)

    with c2:
        state_counts = df.groupby("state")["participant_count"].sum().reset_index()
        st.plotly_chart(px.bar(
            state_counts,
            x="state",
            y="participant_count",
    ...
    with c3:
        ratio = df.groupby("state")["participant_to_provider_ratio"].mean().reset_index()
        st.plotly_chart(px.bar(
            ratio,
            x="state",
            y="participant_to_provider_ratio",
        ...
        st.caption("Higher ratios may indicate provider shortage pressure.")

    with c4:
        gap = df.groupby("state")["utilisation_gap"].mean().reset_index()
        st.plotly_chart(px.bar(
            gap,
            x="state",
            y="utilisation_gap",
        ...
        st.caption("Higher utilisation gaps may indicate that approved supports are not being fully used.")

    scatter_df = df.dropna(subset=["active_provider_count", "participant_count", "utilisation_rate"])
...
with tab4:
    ...
    with col1:
        ...
        disability_type = st.selectbox("Disability Type", sorted(df["disability_type"].dropna().unique()))

    with col2:
        participant_count = st.number_input("Participant Count", min_value=1, value=200, step=50)
        average_support_budget = st.number_input("Average Support Budget", min_value=0, value=25000, step=1000)
        utilisation_rate = st.slider("Plan Utilisation Rate (%)", 0, 100, 85)
        active_provider_count = st.number_input("Active Provider Count", min_value=1, value=80, step=5)
        payment_per_participant = st.number_input("Payment Per Participant", min_value=0, value=20000, step=1000)

    ratio = participant_count / active_provider_count
    gap = 100 - utilisation_rate

    profile = {
        "state": state,
        "age_group": age_group,
        "disability_type": disability_type,
        "support_class": "General Support",
        "participant_count": participant_count,
        "average_support_budget": average_support_budget,
        "utilisation_rate": utilisation_rate,
        "active_provider_count": active_provider_count,
        "participant_to_provider_ratio": ratio,
        "utilisation_gap": gap,
        "payment_per_participant": payment_per_participant
    }
    ...
    m1.metric("Participant-to-Provider Ratio", round(ratio, 2))
    m2.metric("Utilisation Gap", f"{round(gap, 2)}%")
    m3.metric("Payment Per Participant", f"${payment_per_participant:,.0f}")

    if st.button("Generate AI Recommendation"):
        final_risk = classify_access_risk_from_input(profile)
        ...
        with left:
            st.metric("Final Explainable Access Risk", final_risk)
            st.metric("ML Model Prediction", ml_risk)

        with right:
            st.plotly_chart(risk_gauge(final_risk), use_container_width=True)
        ...
        st.subheader("Rule-Based Support Suggestions")
        for s in rule_based_recommendation(profile):
            st.write("- " + s)

        st.subheader("AI Support Recommendation Model")
        support_predictions = get_top_support_predictions(support_model, input_df)
        st.dataframe(support_predictions, use_container_width=True)

        st.subheader("Similar Participant Groups")
        st.dataframe(find_similar_groups(df, profile), use_container_width=True)
...
with tab6:
    ...
    - The model uses public and aggregated data, not individual participant records.
    - The access-risk label is project-defined, not an official NDIS outcome.
    - Some utilisation values may be estimated if source utilisation data is incomplete.
    - The system cannot fully capture lived experience, provider quality, cultural needs, transport barriers, or participant preference.
    """)
    ...
    - The app explains why a risk level is assigned.
    - The final risk output uses transparent rule-based reasoning.
    - ML output is shown as comparison, not as the final authority.
    - Human review is required before any service planning decision.
    """)
